...
Consumers on 7.3.0: remove the `react-native-liquid-glassmorphism` peer, reinstall `@danielsaraldi/react-native-blur-view`, delete any `<Glass>`/`frosted` usage, and re-mount `BlurProvider` where it was dropped. Consumers on ≤ 7.2.x are unaffected — this returns the package to their API surface (now versioned 8.0.0).
