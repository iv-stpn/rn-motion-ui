// AUTO-GENERATED by scripts/gen-icons.mjs — do not edit by hand.
// mingcute icon: apple-intelligence-frame-line · @iconify-json/mingcute v1.2.8
import { Svg, Path as SvgPath, Rect as SvgRect } from 'react-native-svg';
import { useThemeColor } from 'rn-motion-ui/theme/use-theme-color';
import type { IconProps } from '../icon-props';

export function AppleIntelligenceFrameLine({ size = 24, color: colorProp, style, accessibilityLabel, testID }: IconProps) {
  const defaultColor = useThemeColor('foreground');
  const color = colorProp ?? defaultColor;
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      color={color}
      style={style}
      accessibilityRole={accessibilityLabel ? 'image' : undefined}
      accessibilityLabel={accessibilityLabel}
      testID={testID}
    >
      <SvgRect width="16" height="16" x="4" y="4" fill="none" stroke={color} strokeWidth="1.5" rx="3" />
      <SvgPath
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeWidth="1.2"
        d="M11.997 6.375c1.401 0 1.415 1.541 2.722 3.356c1.507 1.786 3.085 2.128 2.763 3.526c-.309 1.334-1.753 1.046-3.743 1.84c-2.185 1.07-2.875 2.608-4.192 1.966c-1.2-.586-.652-1.879-.928-3.91c-.53-2.476-1.948-3.512-1.007-4.674c.89-1.1 2.11-.112 4.383 0c2.276-.11 3.497-1.103 4.388-.003c.94 1.16-.472 2.198-1.003 4.667c-.278 2.036.276 3.332-.928 3.918c-1.314.642-2.006-.89-4.184-1.96c-1.995-.798-3.443-.505-3.752-1.841c-.322-1.396 1.252-1.74 2.757-3.521c1.311-1.818 1.32-3.364 2.724-3.364Z"
      />
    </Svg>
  );
}
