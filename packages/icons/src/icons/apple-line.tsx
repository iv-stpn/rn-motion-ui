// AUTO-GENERATED by scripts/gen-icons.mjs — do not edit by hand.
// mingcute icon: apple-line · @iconify-json/mingcute v1.2.8
import { Svg, Path as SvgPath } from 'react-native-svg';
import type { IconProps } from '../icon-props';
import { useIconColor } from '../resolve-icon-color';

export function AppleLine({ size = 24, color: colorProp, style, accessibilityLabel, testID, ...props }: IconProps) {
  const color = useIconColor(colorProp);
  return (
    <Svg
      {...props}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      color={color}
      style={style}
      accessibilityRole={accessibilityLabel ? 'image' : undefined}
      accessibilityLabel={accessibilityLabel}
      testID={testID}
    >
      <SvgPath
        fill="none"
        stroke={color}
        strokeLinejoin="round"
        strokeWidth="2"
        d="M19.224 9.24a3.501 3.501 0 0 0 .188 6.587c-.822 2.5-2.354 4.556-3.912 5.173c-.541.214-1.3 0-2.046-.212c-.516-.146-1.026-.29-1.454-.29s-.938.144-1.454.29C9.8 21 9.041 21.214 8.5 21c-1.977-.782-3.913-3.884-4.407-7.267c-.414-2.833.494-6.116 3.953-6.698c.804-.135 1.463.14 2.133.421c.567.237 1.142.478 1.821.478s1.254-.24 1.82-.478c.671-.28 1.33-.556 2.133-.42c1.589.266 2.64 1.103 3.271 2.204Z"
      />
      <SvgPath
        fill="none"
        stroke={color}
        strokeLinejoin="round"
        strokeWidth="2"
        d="M12.061 4.757c.377-.069.8-.255 1.12-.575a2.16 2.16 0 0 0 .601-1.308a2.16 2.16 0 0 0-1.308.6a2.16 2.16 0 0 0-.6 1.308q.086-.006.187-.025Z"
        clipRule="evenodd"
      />
    </Svg>
  );
}
