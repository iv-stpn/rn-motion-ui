// AUTO-GENERATED by scripts/gen-icons.mjs — do not edit by hand.
// mingcute icon: rewind-backward-square-30-fill · @iconify-json/mingcute v1.2.8
import { Svg, Path as SvgPath } from 'react-native-svg';
import { useThemeColor } from 'rn-motion-ui/theme/use-theme-color';
import type { IconProps } from '../icon-props';

export function RewindBackwardSquare30Fill({ size = 24, color: colorProp, style, accessibilityLabel, testID }: IconProps) {
  const defaultColor = useThemeColor('foreground');
  const color = colorProp ?? defaultColor;
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      color={color}
      style={style}
      accessibilityRole={accessibilityLabel ? 'image' : undefined}
      accessibilityLabel={accessibilityLabel}
      testID={testID}
    >
      <SvgPath
        fill="none"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="3"
        d="M6.71 9.51c.236-.556.83-1.01 1.49-1.01h.28c.95 0 1.72.77 1.72 1.72c0 .748-.67 1.78-1.5 1.78c.831 0 1.5.981 1.5 1.738a1.7 1.7 0 0 1-1.7 1.7h-.3c-.66 0-1.254-.455-1.49-1.01M3 11v5a3 3 0 0 0 3 3h12a3 3 0 0 0 3-3V8a3 3 0 0 0-3-3H5l2.5-2.5l-1 2m8.95 11a1.75 1.75 0 0 1-1.75-1.75v-3.5a1.75 1.75 0 1 1 3.5 0v3.5a1.75 1.75 0 0 1-1.75 1.75"
      />
    </Svg>
  );
}
