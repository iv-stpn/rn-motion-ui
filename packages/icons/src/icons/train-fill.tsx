// AUTO-GENERATED by scripts/gen-icons.mjs — do not edit by hand.
// mingcute icon: train-fill · @iconify-json/mingcute v1.2.8
import { Svg, Path as SvgPath } from 'react-native-svg';
import type { IconProps } from '../icon-props';
import { useIconColor } from '../resolve-icon-color';

export function TrainFill({ size = 24, color: colorProp, style, accessibilityLabel, testID, ...props }: IconProps) {
  const color = useIconColor(colorProp);
  return (
    <Svg
      {...props}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      color={color}
      style={style}
      accessibilityRole={accessibilityLabel ? 'image' : undefined}
      accessibilityLabel={accessibilityLabel}
      testID={testID}
    >
      <SvgPath
        fill={color}
        fillRule="evenodd"
        d="M1 7c0-1.101.89-2 1.998-2H12c3.224 0 5.942 1.075 7.868 2.589C21.759 9.075 23 11.085 23 13c0 .842-.258 1.56-.713 2.14c-.443.566-1.034.95-1.636 1.214c-1.186.518-2.597.646-3.651.646H2.994A1.995 1.995 0 0 1 1 15zm2 3h4V7H3zm10 0H9V7h3q.51 0 1 .04zm2 0h4.551a8 8 0 0 0-.919-.839c-.962-.756-2.19-1.395-3.632-1.778z"
        clipRule="evenodd"
      />
      <SvgPath fill={color} d="M1 19a1 1 0 0 1 1-1h19a1 1 0 1 1 0 2H2a1 1 0 0 1-1-1" />
    </Svg>
  );
}
