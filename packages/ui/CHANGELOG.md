...
- de42108: Give floating icon buttons a clearer sm/md/lg size ramp and more room around their glyphs. Soften the shared floating shadow, and keep the FAB trigger aligned with its shell.
...
- 369bb65: Match Telegram-style toast geometry with capsule corners by default and larger status icons across sizes on web and native. Keep glass capsules rounded with descriptions and actions; `pill={false}` remains available for rectangular toasts. Tighten title-to-description line spacing and increase icon-to-text separation, emphasize messages with semibold weight, and give capsule text more trailing padding.
...
- 3594360: `ActivityIsland` is now tappable: `ActivityIslandState` accepts an `onPress` so tapping an activity routes to that activity, and `ActivityIsland` accepts an `onPress` fallback for the idle strip and for activities without their own handler. With no handler supplied the strip stays non-interactive.
...
- f5c835c: Add `ActivityIsland` — a full-screen shell with an activity bar above everything in it.
...
- b7abe9b: Default `StatefulButton`'s `variant` to `primary`.
...
- a0ca314: Give the generated file-icon glyphs' numeric attribute values a leading digit, so the paper's gradients survive on native instead of collapsing to flat white.
...
- Updated dependencies [a0ca314]
  - rn-motion-ui-icons@0.0.3
...
- 13eb6a2: Keep the single overlay `Modal` alive when the bottom overlay closes while another is still open.
...
- 7409ea0: fix(FileSystem): stop dropping scroll events on web
...
- fdbd999: fix(MorphingSwitcher): rest the collapsed shell at the shared interactive height
...
- 740a709: revert(Glass): drop react-native-liquid-glassmorphism, restore the blur-view stack
...
- 53956d1: Replace the `@danielsaraldi/react-native-blur-view` optional peer with `react-native-liquid-glassmorphism` and add first-class frosted-glass surfaces.
...
- ebbacd1: Replace the `Button`/`ElevatedButton` loading spinner with three staggered bouncing dots, matching `StatefulButton`'s loader so the whole button family reads consistently while busy. The dots use the same imperative Reanimated loop as the old spinner to avoid the moti `loop` re-render freeze.
- a34f455: feat(menus): expose a uniform `onShow` signal so overlays can autofocus content
...
- 8e4330f: Fix an Android crash when the optional `@danielsaraldi/react-native-blur-view` peer is installed but not autolinked (Bun/pnpm/yarn-berry auto-install optional peers, so the `require()` succeeds while the native `BlurView` ViewManager is never registered).
...
- 19f9d72: Fix `BlurProvider` gating on an unreliable Fabric check that crashes Android release builds.
...
- 61a606b: fix(BlurProvider): don't wrap the app in the peer's BlurTarget on Fabric
...
- 00b4cb6: fix(MorphingFAB): make the collapsed root click-transparent again on web
...
- 54fbd8e: fix(MorphingSwitcher): dismiss on outside press on native + re-tap of the selected item folds the pane
...
- b705147: Add a `variant` prop to `MorphingSwitcher`, plus an `elevation` control and web outside-press close, and give `MenuItem` a `labelWeight` override.
...
- cee40f6: feat(FileSystem): scroll-offset contract — `initialScrollOffset` restore + `onScrollOffsetChange` report
