...
- ab84da1: One shared box for the whole button family, driven by tokens.
  ...
  - **`ElevatedButton`** — padding grows 2–4px per size (10→12, 14→16, 16→20);
    `icon` grows from 32 to 40px. Radii are unchanged (AlignUI's 8/10/12 is what
    the shared ramp was drawn from), and its 14px label is still the documented
    opt-out from the type ramp.
...
- ab84da1: `GlossyButton` labels now use the Button family's type ramp instead
  ...
  unchanged — AlignUI pins its chips to 14px at every size.
