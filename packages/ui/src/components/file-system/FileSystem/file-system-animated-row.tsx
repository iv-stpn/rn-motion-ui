...
export function FileSystemAnimatedRow({ children, height, isEntering, isExiting, onExitComplete }: FileSystemAnimatedRowProps) {
  ...
  // Keep latest isExiting for the onLayout guard (an exiting row must not
  // re-measure and re-animate its height).
  ...
  const onContentLayout = useCallback(
    (event: LayoutChangeEvent) => {
      ...
      // On first layout of an entering row, animate the height from 0 to full.
      if (!(isMeasured.current || isExitingRef.current)) {
    ...
    [containerHeight, height, opacity, reduce],
  );

  return (
    <Animated.View style={containerStyle}>
      {/* onLayout reports the natural content height regardless of the outer's
          animated height (Yoga computes children independently), so entering rows
          always measure their full height. */}
