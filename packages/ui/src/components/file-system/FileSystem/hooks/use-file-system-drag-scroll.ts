...
// backward hop). Now the hook owns a monotonic offset CURSOR, seeded from the
...
export function useFileSystemDragScroll({
  ...
  const runRef = useRef<DragScrollRun | null>(null);
  ...
  useEffect(() => {
    ...
    return cancel;
  }, [cancel, containerRef, isDragging]);
