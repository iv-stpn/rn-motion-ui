...
import { fileTypeFilterGroup, MIME_TYPE_LABELS, mimeTypeForFile, viewerKindForFile } from '../logic/file-system-kinds';
import { normalizeFolderPath, pathName } from '../logic/file-system-paths';
import { flatSearchResults } from '../logic/file-system-search';
import type {
...
export function useFileSystemView() {
  return useStore(useFileSystemStoreContext(), (s) => s.entries.view);
}
...
export function useFileSystemViewActions() {
  return useStore(
    useFileSystemStoreContext(),
    useShallow((s) => ({ setView: s.setView })),
  );
}
...
export function useFileSystemEntriesActions() {
  return useStore(
    useFileSystemStoreContext(),
    useShallow((s) => ({ applySortKey: s.applySortKey, toggleSortColumn: s.toggleSortColumn })),
  );
}
