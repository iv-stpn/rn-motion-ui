 ...
 * `(string & {})` tail lets consumers register arbitrary custom views via
 * {@link FileSystemProps.views} without narrowing the type.
 */
export type FileSystemView = FileSystemBuiltInView | (string & {});
...
export type FileSystemHeaderState = {
  ...
  /** `true` when root width is below the tablet breakpoint (768 px by default, see {@link FileSystemProps.breakpoints}) — the responsive hint a consumer's own switcher can key off. */
  isCompact: boolean;
};
...
export type FileSystemBreakpoints = {
  ...
  compact?: number;
  /** Below this, a consumer's view switcher would typically collapse into a dropdown — a responsive hint, not built-in chrome. @default 768 */
  tablet?: number;
};
...
export type FileSystemProps = {
  ...
  /**
   * Additional views keyed by name, merged over the four built-ins. A key that
   * matches a built-in (`icons`, `list`, `columns`, `gallery`) replaces it, so a
   * consumer can re-skin one without changing the id the store switches on.
   *
   * Each component receives the full {@link FileSystemViewProps} contract — the
   * same flat state the built-in views are handed — so a custom view reuses
   * selection, navigation and drag handling without reaching into the store.
   * Switch to one with `setView` (the `renderHeader` slot, or the
   * `useFileSystemViewActions` hook). While a search query is active the search
   * view replaces whatever view is selected, built-in or custom.
   */
...
/**
 * The flat contract handed to every view — the four built-ins and any custom
 * view registered via {@link FileSystemProps.views} alike. It maps the store's
 * state onto the props a view needs, so a custom view renders the same
 * selection, navigation and drag surface without reaching into the store.
 */
