...
const BUILT_IN_VIEWS: Record<FileSystemBuiltInView, ComponentType<FileSystemViewProps>> = {
...
function FileSystemBodyContent() {
  ...
  // Consumer views win: a `views` key that matches a built-in (e.g. `icons`)
  // replaces it, and any other key becomes a first-class view id.
  const registry: Record<string, ComponentType<FileSystemViewProps>> = { ...BUILT_IN_VIEWS, ...views };
  // The four built-ins are always present, so the lookup only misses on an
  // unregistered custom id; falling back to the default view keeps the dispatch
  // total without asserting a consumer's own registry is complete.
  const ActiveView = isSearching ? FileSystemSearchView : (registry[view] ?? BUILT_IN_VIEWS.icons);
  ...
  return (
    <View
      className={cn(
        ...
        external
          ? 'hairline items-center justify-center border-foreground/20 border-dashed bg-foreground/[0.03]'
          : 'hairline rounded-lg border-info',
      )}
    >
      {external ? (
