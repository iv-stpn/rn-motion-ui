...
function chipClass(isHeld: boolean, isPressed: boolean) {
  if (isHeld) return 'rounded-xl hairline border-primary bg-primary/10 px-6 py-4';
  if (isPressed) return 'rounded-xl hairline border-border bg-surface-2 px-6 py-4 opacity-70';
  return 'rounded-xl hairline border-border bg-surface-2 px-6 py-4';
}
...
function HoldDragDemo({ cursorMode = false, disabled = false, showMenu = false }: HoldDragDemoProps) {
  ...
  const chipBody = (
    <View className="hairline rounded-xl border-border bg-surface-2 px-6 py-4">
