...
function Demo({ onPress }: DemoProps) {
  ...
  return (
    <View className="hairline h-[320px] w-[360px] overflow-hidden rounded-2xl border-border">
      <View className="hairline-b flex-row gap-1.5 border-border p-2">
        {SECTIONS.map((s) => (
          <ScrollTo
            key={s.id}
            scrollRef={scrollRef}
            ...
            onPress={onPress}
            testID={`nav-${s.id}`}
            className="hairline rounded-full border-border bg-surface-3 px-3 py-1"
          >
            {s.label}
          </ScrollTo>
        ))}
