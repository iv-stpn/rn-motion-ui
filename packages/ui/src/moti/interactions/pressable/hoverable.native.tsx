import { Children, type ReactElement, type Ref } from 'react';
import { useSharedValue } from 'react-native-reanimated';
import { HoveredContext } from './hoverable-context';

export type HoverableProps = { children: ReactElement; onHoverIn?: () => void; onHoverOut?: () => void; childRef?: Ref<unknown> };

// On native, hovering is not supported — provide a no-op context value.
export function Hoverable({ children }: HoverableProps) {
  return <HoveredContext.Provider value={useSharedValue(false)}>{Children.only(children)}</HoveredContext.Provider>;
}
